...
class DeepseekV3MoE(nn.Module):
    """Mixed expert module containing shared experts."""
...
class DeepSeekV3Indexer(nn.Module):
    ...
    def forward(
        ...
        input_dtype = hidden_states.dtype
        k = self.k_norm(self.wk(hidden_states).to(torch.float32)).to(input_dtype).unsqueeze(1)
        k_pe, k_nope = torch.split(k, [self.qk_rope_head_dim, self.head_dim - self.qk_rope_head_dim], dim=-1)
        # print(sin.shape,cos.shape,q_pe.shape,k_pe.shape)
        ...
        scores = torch.einsum("bhsd,bxtd->bsht", q.float(), k_cached.float()) * self.softmax_scale
...
class DeepseekV3Attention(nn.Module):
    ...
    def __init__(self, config: DeepseekV3Config, layer_idx: Optional[int] = None):
        ...
        self.indexer = DeepSeekV3Indexer(config)
        ...
        if self.config.rope_scaling is None:
            ...
            if scaling_type == "linear":
                self.rotary_emb = DeepseekV3LinearScalingRotaryEmbedding(
                    self.qk_rope_head_dim, max_position_embeddings=self.max_position_embeddings,
                ...
                self.rotary_emb = DeepseekV3YarnRotaryEmbedding(
                    self.qk_rope_head_dim, max_position_embeddings=self.max_position_embeddings,
                    scaling_factor=scaling_factor, base=self.rope_theta, **kwargs,
                )
            else:
    ...
    def forward(
        ...
        if past_key_value is not None:
            if self.layer_idx is None:
                raise ValueError("Cache requires layer_idx to be set.")
        ...
        # print(past_key_value, q_pe.shape, k_pe.shape)
        if past_key_value is not None:
        ...
        if combined_mask is not None:
            if combined_mask.size() != (bsz, 1, q_len, kv_seq_len):
                raise ValueError(f"Attention mask shape mismatch: {combined_mask.size()}")
...
class DeepseekV3FlashAttention2(DeepseekV3Attention):
    """DeepSeek-V3 flash attention module."""
    ...
    def forward(
        ...
        if input_dtype == torch.float32:
            ...
            logger.warning_once(f"Input hidden states silently casted to float32. Casting back to {target_dtype}.")
        ...
        attn_output = self._flash_attention_forward(
            ...
            dropout=dropout_rate, softmax_scale=self.softmax_scale,
        )
        if self.q_head_dim != self.v_head_dim:
            attn_output = attn_output[:, :, :, : self.v_head_dim]
...
class DeepseekV3DecoderLayer(nn.Module):
    ...
    def forward(
        ...
        output_attentions: Optional[bool] = False, use_cache: Optional[bool] = False, **kwargs,
...
DeepseekV3_INPUTS_DOCSTRING = r"""
    Args:
        ...
        past_key_values (`Cache` or `tuple(tuple(torch.FloatTensor))`, *optional*):
            Pre-computed hidden-states for speeding up sequential decoding.
        inputs_embeds (`torch.FloatTensor` of shape `(batch_size, sequence_length, hidden_size)`, *optional*):
            Optionally, instead of passing `input_ids` you can pass an embedded representation directly.
        use_cache (`bool`, *optional*): If set to `True`, past key value states are returned.
        output_attentions (`bool`, *optional*): Whether to return attention tensors.
...
class DeepseekV3ForSequenceClassification(DeepseekV3PreTrainedModel):
    ...
    def forward(
        ...
        return SequenceClassifierOutputWithPast(
            loss=loss, logits=pooled_logits, past_key_values=transformer_outputs.past_key_values,
